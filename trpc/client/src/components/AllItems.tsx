...
function AllItems() {
  ...
  if (allFlora.isLoading) return <>Loading</>;
  if (allFlora.isError)
    return (
  ...
  return (
    <>
      <ul
        className={`grid gap-y-3 transition-opacity${
          allFlora.isRefetching ? " opacity-50" : ""
        }`}
      >
        {[...allFlora.data].reverse().map((flora) => (
